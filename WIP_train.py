...
def setup(cfg, data_path:str, device: torch.device):
    ...
    try:
        ...
        val_loader = DataLoader(val_dataset, batch_size=cfg['batch_size'], shuffle=True, num_workers=cfg['num_workers']) # Change Batch size
...
def main():
    ...
    # It is used to initiate a connection to Google Cloud if it is present, otherwsise it just upload locally
    ...
    # Handler to resume where interrupted if happened both with the distiction between Google Cloud and local
    if resume_ckpt_cli:
    ...
    num_workers    = get_param('num_workers', args.num_workers)
    ...
    patience = cfg['early_stopping_patience'] or 15
    ...
    if resume_ckpt_local:
        ...
        remaining_epochs = args.epochs if args.epochs else cfg['epochs']
